...
bool RC6::Decrypt(const uint8_t * cipher, uint8_t * plain)
{
	...
	rc6_word D = *reinterpret_cast<rc6_word*>(const_cast<uint8_t*>(&cipher[12]));
...
bool RC6::Setup()
{
	...
	rc6_word A = 0, B = 0;
	for (size_t i = 0, j = 0, k = 0; k < 3 * c_rc6t; ++k, i = (i + 1) % c_rc6t, j = (j + 1) % c_rc6c)
	{
