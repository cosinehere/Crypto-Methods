...
inline uint8_t ffm_ef(uint8_t x) { return ((x) ^ ((x) >> 1) ^ ((x) >> 2) ^ tab_ef[(x) & 3]); }
...
{
/*#    define g0_fun(x)*/
	return (mk_tab[0][byte(x, 0)] ^ mk_tab[1][byte(x, 1)] ^ mk_tab[2][byte(x, 2)] ^ mk_tab[3][byte(x, 3)]);
}
inline uint32_t g1_fun(uint32_t x)
{
/*#    define g1_fun(x)*/
	return (mk_tab[0][byte(x, 3)] ^ mk_tab[1][byte(x, 0)] ^ mk_tab[2][byte(x, 1)] ^ mk_tab[3][byte(x, 2)]);
}
#  else
...
const size_t Twofish::BlockSize()
{
	return p_blocksize;
}
