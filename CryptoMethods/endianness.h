...
static inline float bswapf(float f) {
	...
	uint32_t asInt;
	memcpy(&asInt, reinterpret_cast<const void *>(&f), sizeof(uint32_t));
	asInt = bswap32(asInt);
	memcpy(&f, reinterpret_cast<void *>(&asInt), sizeof(float));
	return f;
#else
...
static inline double bswapd(double d) {
	...
	uint64_t asInt;
	memcpy(&asInt, reinterpret_cast<const void *>(&d), sizeof(uint64_t));
	asInt = bswap64(asInt);
	memcpy(&d, reinterpret_cast<void *>(&asInt), sizeof(double));
	return d;
#else
