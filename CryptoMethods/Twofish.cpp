...
#define q(n, x) c_Qtab[n][x]  // q_tab[n][x]
...
#define fm_33 ffm_5b
#define q_3(x) q(0, x)
#define f_0(n, x) ((uint32_t)fm_0##n(x))
#define f_1(n, x) ((uint32_t)fm_1##n(x) << 8)
#define f_2(n, x) ((uint32_t)fm_2##n(x) << 16)
#define f_3(n, x) ((uint32_t)fm_3##n(x) << 24)
#define mds(n, x) \
    f_0(n, q_##n(x)) ^ f_1(n, q_##n(x)) ^ f_2(n, q_##n(x)) ^ f_3(n, q_##n(x))
...
#ifndef M_TABLE
    uint32_t m5b_b0, m5b_b1, m5b_b2, m5b_b3;
    uint32_t mef_b0, mef_b1, mef_b2, mef_b3;
#endif
...
#define q30(x) \
    q(0, q(0, q(1, x) ^ byte(key[2], 0)) ^ byte(key[1], 0)) ^ byte(key[0], 0)
#define q31(x) \
    q(0, q(1, q(1, x) ^ byte(key[2], 1)) ^ byte(key[1], 1)) ^ byte(key[0], 1)
#define q32(x) \
    q(1, q(0, q(0, x) ^ byte(key[2], 2)) ^ byte(key[1], 2)) ^ byte(key[0], 2)
#define q33(x) \
...
#else
#define g0_fun(x)                                            \
    (mds(0, sb[0][byte(x, 0)]) ^ mds(1, sb[1][byte(x, 1)]) ^ \
     mds(2, sb[2][byte(x, 2)]) ^ mds(3, sb[3][byte(x, 3)]))
#define g1_fun(x)                                            \
    (mds(0, sb[0][byte(x, 3)]) ^ mds(1, sb[1][byte(x, 0)]) ^ \
